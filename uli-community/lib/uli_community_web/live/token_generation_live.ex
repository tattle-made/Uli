defmodule UliCommunityWeb.TokenGeneratorLive do
  ...
  def mount(_params, _session, socket) do
    ...
    tokens =
      Api.get_access_tokens_by_user_id(socket.assigns.scope)
      |> Enum.map(
        &Map.take(&1, [
          :id,
          :token_id,
  ...
  def handle_event("gen_token", %{"token" => params}, socket) do
    ...
    with {:ok, added_entry} <- Api.add_new_access_token(socket.assigns.scope, attrs),
  ...
  def handle_event("delete_token", %{"token" => token}, socket) do
    case Api.delete_access_token_by_token_id(socket.assigns.scope, token["token_id"]) do
      {:ok, deleted_token} ->
        updated_rows =
