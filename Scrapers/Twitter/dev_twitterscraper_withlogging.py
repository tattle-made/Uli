...
if __name__ == "__main__":
    ...
    if args.hashtags:
        ...
        hasha = str(args.year) + '_' +args.hashtags
        new_folder = FOLDER/'hashtags'/hasha
        logging.info(f'{new_folder}')

    elif args.keywords:

        hasha = str(args.year) + '_' + args.keywords 
        new_folder = FOLDER/'keyw0rds'/hasha
        logging.info(f'{new_folder}')

    elif args.handles:

        hasha = str(args.year) + '_' +args.handles
        new_folder =FOLDER/'handles'/hasha
        logging.info(f'{new_folder}')
    ...
    for idx,month in enumerate(month_dict.values()):
        ...
        start_date = date(args.year,idx+1,month[0])
        end_date = date(args.year,idx+1,month[1])
    
        for single_date in daterange(start_date, end_date):
            until = single_date.strftime("%Y-%m-%d")
            print(until)
            time.sleep(1)
            upload_folder = str(new_folder) + '\t' + str(until)            
