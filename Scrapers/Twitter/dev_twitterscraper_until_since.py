...
def scraper_util(hashtag,handle,keyword,limit,until,since):
    ...
    if hashtag:
        ...
        c.Search = "#" + hashtag
        c.Output = c.Until + hashtag + ".json"

    elif handle:   
        print("Handle")
        print(handle)
        c.Username = handle
        c.Output = c.Until + handle + ".json" 
    ...
    end_note = c.Output
    logging.info(f"Scraping done : {end_note}!!!!!")
...
def run_scraper(hashtags,handles,keywords,limit,until,since):
    ...
    if hashtags:
        ...
        for hashtag in hashtags:
            scraper_util(hashtag,handles,keywords,limit,until,since)
        ...
        for keyword in keywords:
            scraper_util(hashtags,handles,keyword,limit,until,since)
...
def helper(search_term):


        with open(search_term,'r') as txt:

            terms_list = txt.readlines()
            search_term = search_term.split('.')[0]
            print(search_term)
            search = '1819' +  search_term

            new_folder = FOLDER/search
            ...
            for term in terms_list:
                ...
                scrap_note = "Scraping : " + term
                logging.info(f'{scrap_note}')
                
                if search_term == 'handle':
                    t = threading.Thread(target=run_scraper,args=(args.hashtags,term,args.keywords,args.max,args.until,args.since,))
                elif search_term == 'keyword':
                    t = threading.Thread(target=run_scraper,args=(args.hashtags,args.handles,term,args.max,args.until,args.since,))
                elif search_term == 'hashtag':
                    t = threading.Thread(target=run_scraper,args=(term,args.handles,args.keywords,args.max,args.until,args.since,))

                t.start()

                PATH = os.getcwd()
                print(PATH)
                end_note = 'Scraping done : ' + str(time.strftime("%Y%m%d%H%M%S"))  + term
                logging.info(f'{end_note}')

                #time.join()
                #time.sleep(5)

            end_note = 'Final Scraping done : ' + str(time.strftime("%Y%m%d%H%M%S"))  + term
            logging.info(f'{end_note}')  

if __name__ == "__main__":

    logging.basicConfig(filename='twitterscraper2019.log', level=logging.INFO)
    ...
    if args.hashtags:
        ...
        helper(args.hashtags)

    elif args.keywords:

        helper(args.keywords)
